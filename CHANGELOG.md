...
- **The fix.** Drop the pinned mint clock so the token is minted at the real `now` (matching every other mint-then-validate test in the file, which never pin). The load-bearing assertion is `exp - iat === OPERATOR_TOKEN_TTL_SECONDS`, which is invariant of the mint instant, so it survives unchanged. The test name, which claimed "~1y TTL," is corrected to "90-day TTL" (the constant has been 90d since #213). An on-site comment records why pinning the mint clock here is the trap.
- **Sibling sweep — the valuable part.** Every hardcoded absolute-date literal in `src/__tests__/` was audited for the same shape (pinned mint + real-clock validate). **Exactly one bomb existed — the one above.** All other pinned dates are inert, for three structural reasons: (1) the hub's own expiry logic (invites, auth-codes, sessions, refresh-token rotation, rate-limit, account descriptors, uptime) is uniformly **clock-injectable**, and the tests inject `now` on **both** the write and the check, so they're self-consistent; (2) TTL assertions compare a pinned mint against the **same** pinned clock arithmetically (`jwt-sign.ts` refresh-TTL, `surface-token.ts` 90d default), never the real clock; (3) tests that pin a past mint and **expect** expiry (`operator-token.ts`'s fully-expired case, the self-heal `unverifiable` case) stay expired forever. The only real-clock enforcement any test **arms** is jose's `exp`, which is why a **long-TTL** token (operator 90d, surface 90d) minted at a pinned past date is the sole shape that can hide a burning fuse; the surface-token 90d mint never reaches a validator, leaving operator-token as the one armed instance. (There is a second wall-clock path — `getAllPublicKeys` (`signing-keys.ts`) drops a signing key from JWKS once its `retired_at` is older than the ~24h retention window, so `validateAccessToken` can't find a long-retired key's `kid` — but no test arms it: active keys have `retired_at` NULL and are never age-filtered, and every test signs with a fresh `rotateSigningKey`.)
- **Recurrence guard.** Deliberately a comment at the fix site, not a lint rule or meta-test: a scanner for "pinned mint" would false-positive against the many legitimate inject-`now`-on-both-sides tests, and a lint framework is out of proportion to a one-instance class.
- No migration file — test-only change; no persisted shape or wire changes.

## [0.7.8-rc.4] - 2026-07-28

**fix(notes-serve): stop the supervisor's `cwd=/` from letting bun's install cache shadow a bun-linked checkout — a silent version rollback (#780).** A shim-served front-end (notes/app) is launched by the supervisor with **cwd = `/`**. `resolveNotesDistFrom` tried `process.cwd()` as its *first* resolution candidate, and from `/`, `Bun.resolveSync` finds nothing up the tree and falls through to bun's own install *cache* (`~/.bun/install/cache/<pkg>@<ver>@@@1`) — a copy laid down by any `bun add`/install. That cached copy then won *before* the global link was ever consulted, so a bun-linked checkout was silently ignored in favour of an arbitrarily old published version. Observed live on 2026-07-28 (issue filed 07-27): after `@openparachute/app` was renamed, hub served npm `@openparachute/app@0.22.5` for nine hours while `parachute status` reported `bun-linked → parachute-app @ <sha>` — the status line was true about *resolution* and false about *what was served*, which is exactly what made it take so long to find.

- **The fix — drop `/` as a resolution candidate.** `notesDistCandidates` now omits `cwd` when it is `/`, forcing resolution through the global link table (`~/.bun/install/global/node_modules`, then the older global root) — the intended source for a supervised service. A real checkout cwd is never `/`, so nothing legitimate is lost; a shim invoked from inside a package checkout (cwd ≠ `/`) still resolves cwd-first exactly as before. This is deliberately *not* the "launch children with a different cwd" alternative — services resolve relative to cwd today and that change would have a far larger blast radius.
- **The guardrail — make the failure self-evident in `status`.** New `detectServedResolution` resolves a shim-served package the way notes-serve does at serve time and compares it against the package's bun-global link. When they diverge, `parachute status` prints a `! SERVED-DRIFT: serving <pkg>@<version> from <served-path>, not the global link at <linked-path> — …` continuation line (mirroring the existing `STALE:` note shape), reporting the version read from the path actually served from. This turns a multi-hour asset-hash trace into a glance. Post-fix, resolution lands on the global link, so the note stays silent — it fires only when resolution genuinely lands off the link (a stale cache entry, a broken/renamed link), belt-and-suspenders to the candidate-list fix.
- Tests: a regression test drives `resolveNotesDistFrom` with cwd `/` + a resolvable install-cache entry and pins that resolution takes the global link, never the cache (fails on the pre-fix candidate list); `notesDistCandidates("/", …)` omits `cwd`; and the guardrail's divergent / consistent / no-link / unresolvable / unreadable cases are covered against injected resolution seams.
- No migration file — no persisted shape changes; this only affects package resolution at serve time and a diagnostic continuation line in `status`.

## [0.7.8-rc.3] - 2026-07-27

**fix(oauth-client): recognize the remote root-form MCP URL for outbound-grant scope derivation, gated on a Parachute signal (#775).** `deriveVaultScopeFromMcpUrl` only recognized the URL-addressed `…/vault/<name>/mcp` shape; a remote's canonical root `…/mcp` (the form PR #774 just made the advertised canonical URL for self-hosted, matching Cloud) fell through the null path to the resource's full advertised `scopes_supported` — the broad pair `vault:read vault:write`, bypassing the least-privilege posture #671 established. Root form is now recognized, but resolves differently: it has no vault name in the path to narrow against (the token's audience carries it instead), so requesting the broad unnamed pair IS correct — the remote's own consent picker narrows it to one vault at authorize time. The new `ROOT_MCP_PATH_RE` (anchored `^/mcp/?$`) makes that an explicit, commented branch rather than an accidental null-fallback; `VAULT_MCP_PATH_RE`'s URL-addressed derivation is unchanged and still wins when a name is present.
- **Gated on an actual Parachute signal, not just the path shape.** Bare `/mcp` is ALSO the generic industry MCP convention — most third-party MCP servers live at exactly `/mcp` — so root form alone isn't evidence of a Parachute remote. `deriveVaultScopeFromMcpUrl` now takes the caller's already-discovered `scopesSupported` as a second argument; the root branch only fires when `advertisesVaultScopes` finds a `vault:`-prefixed scope in it. A non-Parachute server at root `/mcp` falls through to `null` exactly like before, keeping the existing `scopes_supported` fallback intact for it. `admin-agent-grants.ts` threads `discovery.scopesSupported` through at its one call site; its scope-selection comment updated to describe both branches and the gate.

- Tests: root form (with/without trailing slash, with query/fragment) + a Parachute signal → the broad pair via the deliberate path; root form WITHOUT the signal (a generic MCP server, or no `scopesSupported` at all) → `null`; a near-miss (`/notmcp`) never matches root, signal or not; URL-addressed form still derives `vault:<name>:read` regardless of signal (regression guard). Two pre-existing tests that used `https://remote.test/mcp` as a generic "non-vault" stand-in were repointed to `https://remote.test/some/other/mcp` (genuinely non-vault, non-root) since root form now resolves differently; grant-flow-level tests pin both a Parachute remote at root `/mcp` (broad pair, not its advertised `vault:eng:*`) and a generic remote at root `/mcp` (falls back to its own advertised `scopes_supported`, no Parachute scope handed to a server that never asked for one).
...
- **CLI setup-wizard no longer busy-hangs on a non-interactive stdin.** `parachute setup-wizard`'s transcription step reached an interactive `prompt("Pick [1]:")` and Bun's `readline/promises` `question()` NEVER settles when stdin is closed (run.sh execs stages.sh without a TTY; also cloud-init, `ssh host 'parachute setup-wizard …'`), busy-hanging forever until the outer 25-min job cancel. Now: `wizard.ts`'s `defaultPrompt` FAILS FAST with a flag-naming error when `!process.stdin.isTTY` (account / vault / expose are required steps), and `wizard-transcription.ts` DEFAULTS the optional transcription step to `none` with an honest log line on a non-TTY stdin (never blocks setup). `setupFetch` also gained a `AbortSignal.timeout(30_000)` per-request ceiling so a wedged hub call trips instead of suspending the op-poll deadline (latent, belt-and-suspenders).
- **The wizard's vault step is no longer silently skipped on an init'd box (`--vault-mode create --vault-name` was being dropped).** `deriveWizardState`'s `hasRealVault` discriminated only on `version !== SEED_VERSION`, but the hub's boot-spawned vault module child self-registers its services.json row at the REAL package version with `paths: []` (no instance yet) — defeating the sentinel, so the wizard saw `hasVault: true` and jumped to expose, creating nothing. `hasRealVault` is now INSTANCE-aware: a real vault also requires `paths.length > 0` (the same #478 empty-paths discrimination `findExistingVault` / `listVaultInstanceNames` already use). The three consumers of `hasRealVault` (`deriveWizardState`, `handleSetupGet`'s `?step=vault` re-entry gate, `handleSetupVaultPost`'s already-provisioned short-circuit) all read the same derived value, so they move together. Additionally, `handleSetupVaultPost`'s already-supervised short-circuit — which previously SYNTHESIZED a succeeded op without creating anything when the vault module child was already supervised (the boot-spawned case) — now drives `provisionVault(name, …)` (== the shipped admin-SPA `parachute-vault create <name> --json` path, idempotent + name-validated) **fire-and-forget** (mirrors the normal `void runInstall` path: returns the op_id immediately, marks the op from the background result, so the CLI's new 30s per-request ceiling only bounds the quick POST/poll round-trip, never a slow cold `parachute-vault create`). A `findService` pre-check avoids a second `parachute install vault` in the narrow pre-seed window of a concurrent create POST.
- **Tier-1 e2e harness headless-hardening.** `e2e/stages.sh` passes `--transcribe-mode none` to the wizard invocation and wraps it in `timeout 420` (real exit code captured via `|| rc=$?`), dumping `journalctl -u parachute-hub` + `parachute status` on failure/timeout — so a future non-TTY wedge trips in 7 minutes with diagnostics instead of hanging to the job cancel (which never reached the failure branch). The existing "Setup complete" / "admin account created" / "Vault ready" assertions are preserved.

### Changed

- **`parachute setup-wizard` now requires flags (not piped stdin) for non-interactive runs.** Because `defaultPrompt` fails fast on a non-TTY stdin, feeding answers via a pipe/heredoc (`printf 'owner\n…' | parachute setup-wizard`) no longer works — it throws with a message naming the flag to pass. The sanctioned non-interactive path is the paired flags (`--account-username` / `--account-password` / `--vault-mode` / `--vault-name` / `--expose-mode` / `--transcribe-mode`), which are unambiguous and don't depend on prompt ordering. Interactive terminal use is unchanged.

### Known limitations

- **Vault *import* during setup is rejected on a box whose vault module is already supervised** (the normal init'd-box path). The already-supervised create path provisions an empty vault instance; wiring the mirror-import follow-up through it isn't exercised by the e2e and couldn't be verified end-to-end, so `handleSetupVaultPost` returns an actionable error (a FAILED op + "importing a vault during setup isn't supported yet on a box whose vault module is already running. Create the vault here instead, then import into it with `parachute vault …` or the admin UI.") rather than silently creating an empty vault named after the import target. Create-during-setup + import-after is the supported flow until the follow-up lands.
