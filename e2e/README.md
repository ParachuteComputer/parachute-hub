...
`PASS` / `FAIL` / `SKIP` as usual, plus **`XFAIL`** — a *known, tracked, reported-in-the-PR* live bug the harness deliberately surfaces. One is XFAIL'd today, gated on its specific signature so a *different* failure still hard-FAILs:
...
> **`stage4-wipe-recovery` flipped XFAIL(#610) → PASS in this PR.** The proactive watchdog now self-recovers automatically, so the stage asserts that recovery happens with no operator nudge; a hub that does *not* self-recover is a real `FAIL` (#610 regression), not an XFAIL.

An XFAIL is loud in the summary but does **not** fail the suite (so the harness stays usable as a gate while screaming the finding). A new/unexpected failure always `FAIL`s and aborts with a non-zero exit. `run.sh` exits 0 with a warning banner when only XFAILs are present, non-zero on any FAIL. (The remaining XFAIL flips back to a hard assertion once its fix lands — vault#464 in a separate PR.)
