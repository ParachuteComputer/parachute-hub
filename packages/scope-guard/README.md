...
- **`createScopeGuard({ hubOrigin, jwksOrigin?, jwks?, jwksGetter?, allowMissingJti?, missingJtiLogger? })`** — factory bound to a hub origin. Holds the JWKS getter so the cache lives across requests. `hubOrigin` may be a string or a resolver function (for layered env-var precedence). `jwksOrigin` (same shape, defaults to `hubOrigin`) FETCHES the JWKS from a different origin than the one `iss` is validated against — a co-located resource server behind a public tunnel can validate `iss` against the public FQDN while reading its hub's keys from loopback, avoiding a tunnel hairpin (vault#464). `allowMissingJti` (default `false` — strict) controls whether hub-signed JWTs lacking a `jti` claim are accepted (see [Versioning](#versioning) for the 0.4.0 rationale).
- **`guard.validateHubJwt(token, { expectedAudience? })`** — JWKS-backed verify. Pins `iss` to the configured hub origin, strict-checks `aud` (RFC 7519 string-or-array) when supplied. Rejects hub-signed tokens lacking `jti` by default (since 0.4.0; see CHANGELOG for the opt-out). On a rotation-class failure (`signature` / `kid` / `jwks`) it force-reloads the JWKS and retries once — recovering from same-kid rotation, no-kid-multi-key, and within-`cacheMaxAge` staleness that jose's reactive reload can't reach (forced reloads are throttled by `jwksReloadMinIntervalMs`, default 10s). Throws `HubJwtError` (with a `code`) on failure.
